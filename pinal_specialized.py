    ...
    with upload_tab:
        ...
        if uploaded_file is not None:
            try:
                ...
                required_columns = ["ID", "Title", "Priority", "Original Estimates", "Category"]
                ...
                if missing_columns:
                    ...
                    # Analyze components from titles
                    df["Component"] = df["Title"].str.extract(r"(Comp\d+)", expand=False)
                    component_counts = df["Component"].value_counts()

                    # Analyze effort by category and priority
                    effort_by_category = df.groupby("Category")["Original Estimates"].sum()
                    effort_by_priority = df.groupby("Priority")["Original Estimates"].sum()
                    ...
                    with col1:
                        ...
                        st.write(pd.DataFrame({
                            ...
                            "Percentage": (priority_counts.values / total_tasks * 100).round(1)
                        }))
                    ...
                    with col2:
                        ...
                        st.write(pd.DataFrame({
                            ...
                            "Percentage": (category_counts.values / total_tasks * 100).round(1)
                        }))
                    ...
                    st.write(pd.DataFrame({
                        ...
                        "Percentage": (component_counts.values / total_tasks * 100).round(1)
                    }))
                    ...
                    with col3:
                        st.write("**Effort by Category (hours)**")
                        st.write(pd.DataFrame({
                            "Category": effort_by_category.index,
                            "Total Hours": effort_by_category.values.round(1)
                        }))

                    with col4:
                        st.write("**Effort by Priority (hours)**")
                        st.write(pd.DataFrame({
                            "Priority": effort_by_priority.index,
                            "Total Hours": effort_by_priority.values.round(1)
                        }))
                ...
                st.error("Please make sure your CSV file has the required columns (ID, Title, Priority, Original Estimates, Category)")
        else:
